...
export const GET = withProtection(
  ...
  {
    requiredRoles: ["admin"],
    activity: "Melihat Data User Berdasarkan ID",
  }
);
    ...
    try {
      ...
      if (id_role && Array.isArray(id_role) && id_role.length > 0) {
        ...
        for (const roleId of id_role) {
          const role = await roleRepo.findOneBy({ id_role: roleId });
          if (!role) continue;
      ...
      const updatedUser = await ds.getRepository(User).findOne({
        where: { id_user: id },
        relations: ["userRoles", "userRoles.role"],
      });
